// The natural logthis.#arithm of 10 (1025 digits).
const LN10 =
  '2.';
// The natural logarithm of 2 (1025 digits).
const LN2 =
  '0.';
const PI =
  '3.';
const E =
  '2.';

const numerals = '0123456789abcdefghijklmnopqrstuvwxyz';

const MAX_FLOAT_VALUE =
  '11111111111111111111111011111111100000111000000110101100100101000111000000100011101001111001111001000000000000000000000000000000';
const MAX_DOUBLE_VALUE =
  '' +
  '0'.repeat(293);

// FUNCTIONS DECLARED AS B prototypical ----------------------------------------------------------------------------------------------------------
class BaseNumber {
  static Ln10 = new BaseNumber(LN10, 10, 1024);
  static Ln2 = new BaseNumber(LN2, 10, 1024);
  static Pi = new BaseNumber(PI, 10, 1024);
  static e = new BaseNumber(E, 10, 1024);
  #decimals;
  #shift;
  #angles;
  #b;
  #n;
  #f;
  #s;
  /**
   * @param {*} n = ?0
   * @param {*} base = ?10
   * @param {*} decimals = ?20
   * @returns fresh new BigDecimal
   */
  constructor(n = 0, base = 10, decimals = 20) {
    this.setDecimals(decimals);
    this.#angles = 'degrees';
    if (!(this instanceof BaseNumber)) return new BaseNumber(n, base);
    if (n instanceof BaseNumber) return n.clone();
    if (n instanceof BigInt) n = n.toString();

    n = Normalize(n);
    base = parseInt(base, 10);

    _catch(n, base, "'constructor()'");
    n = fixNott(n);
    this.#n = n;
    this.#b = base;
    this.#f = n.indexOf('.') + 1;
    this.#s = n[0] === '-';
    /*
    console.log(
      '[ ',
      'n: ',
      this.#n,
      'b: ',
      this.#b,
      'f: ',
      this.#f,
      's: ',
      this.#s,
      'valueOf: ',
      this.valueOf(),
      ' ]',
    ); */
  }

  get decimals() {
    return this.#decimals;
  }
  set decimals(value) {
    this.#decimals = value;
  }
  get shift() {
    return this.#shift;
  }
  set shift(value) {
    this.#shift = value;
  }
  get angles() {
    return this.#angles;
  }
  set angles(value) {
    this.#angles = value;
  }
  get b() {
    return this.#b;
  }
  set b(value) {
    this.#b = value;
  }
  get n() {
    return this.#n;
  }
  set n(value) {
    this.#n = value;
  }

  /** @functionBigInteger arithmetic
   *
   * BigInt is a built-in library that allows user to make operations with big integers.
   * However, there isn´t something available for BigDecimals.
   * This implements a BigDecimal inside BaseNumber.js
   *
   * @param (string, string, string)
   * @return the result of the operation
   */
  ___arith(b1, b2, op) {
    return this.#arith(b1, b2, op);
  }
  #arith(b1, b2, op) {
    // Fix precision to what is needed
    b1 = b1.split('.').concat('');
    b1[1] = b1[1].slice(0, this.#decimals);
    b1 = b1[0] + (b1[1].length ? '.' + b1[1] : '');

    b2 = b2.split('.').concat('');
    b2[1] = b2[1].slice(0, this.#decimals);
    b2 = b2[0] + (b2[1].length ? '.' + b2[1] : '');

    if (op === 'pow') return fixNott(_pow(b1, b2, this.#decimals, this));
    if (op === 'root')
      return fixNott(_pow(b1, this.#arith('1', b2, 'd'), this.#decimals, this));

    // Add point at the end in case there isn't
    b1 += b1.indexOf('.') < 0 ? '.' : '';
    b2 += b2.indexOf('.') < 0 ? '.' : '';

    // Add '0' on the right
    b1 = b1.padEnd(b1.indexOf('.') + b2.length - b2.indexOf('.'), '0');
    b2 = b2.padEnd(b2.indexOf('.') + b1.length - b1.indexOf('.'), '0');

    // Get dot index
    const dot = b1.indexOf('.');

    // Erase dots
    b1 = b1.replace('.', '');
    b2 = b2.replace('.', '');

    // Decimals after comma
    const dec = Math.abs(dot - b1.length);

    // In order to move the comma we need to remove the sign to avoid errors like '0000-0.56'
    function fixDot(s, cond) {
      const sign = s[0] === '-' ? '-' : '';
      sign && (s = s.slice(1));
      s = s.padStart(cond, '0');
      return sign + s.slice(0, s.length - cond) + '.' + s.slice(s.length - cond);
    }

    if (op === 'a') return fixNott(fixDot(String(BigInt(b1) + BigInt(b2)), dec));
    if (op === 's') return fixNott(fixDot(String(BigInt(b1) - BigInt(b2)), dec));
    if (op === 'm') return fixNott(fixDot(String(BigInt(b1) * BigInt(b2)), dec * 2));
    if (op === 'd')
      return fixNott(
        fixDot(String(BigInt(b1 + this.#shift) / BigInt(b2)), this.#decimals + 1),
      );
    if (op === 'mod') return fixNott(fixDot(String(BigInt(b1) % BigInt(b2)), dec));
  }
  ___natLog(x) {
    return this.#naturalLogarithm(x);
  }
  ___natExp(x) {
    return this.#naturalExponential(x);
  }
  #naturalLogarithm(x) {
    /* Series converges faster when argument is closer to 1, so let's reduce the argument the way that
     * ln(x) = ln(a*10^b)
     * = ln(a) + b * ln(10)
     */
    let r = '0',
      a = x.replace('.', ''),
      b = (x.indexOf('.') + 1 || x.length + 1) - 2;
    // If number <= 0.9
    if (cmp(x, '0.9') != 1) {
      // Trail zeros at left to convert any decimal <= 0.9 into an integer
      for (; a[0] === '0'; b--) a = a.slice(1);
    }
    a = a[0] + '.' + a.slice(1);

    // If a is higher or equal to 1.1
    if (cmp(a, '1.1') > -1) {
      /* Reduce the argument again using ln(2) the way that
       * ln(a) = ln(r*2^y)
       * = ln(r) + y * ln(2)
       */
      let y = Math.floor(a / 2) - 1;
      y < 1 && (y = 1);
      const d = String(2 ** y);
      r = this.#arith(
        this.#naturalLogarithm(this.#arith(a, d, 'd')),
        this.#arith(String(y), LN2, 'm'),
        'a',
      );
    } else {
      // Take logthis.#arithm
      let aux = this.#arith(this.#arith(a, '1', 's'), this.#arith(a, '1', 'a'), 'd'),
        acc = aux,
        sq = this.#arith(aux, aux, 'm'),
        last = 'x',
        cur = 0;
      // Use Taylor series
      for (let i = 1; cur <= this.#decimals; i += 2) {
        last = r;
        r = this.#arith(r, this.#arith(acc, String(i), 'd'), 'a');
        acc = this.#arith(acc, sq, 'm');
        // Search for a digit that is not accurate
        for (; last[cur] === r[cur] && cur <= this.#decimals; cur++);
      }
      r = this.#arith(r, '2', 'm');
    }

    return this.#arith(r, this.#arith(String(b), LN10, 'm'), 'a');
  }
  #naturalExponential(x) {
    /* Argument reduction
     * if x > LN2, do
     * exp(x) = exp(a) * 2^b  where a = x - b * ln2, b = floor(x / ln2)
     */
    if (cmp(x, LN2) == 1) {
      // Do floor
      let b = this.#arith(x, LN2, 'd');
      // if b has decimals
      if (b.indexOf('.') + 1) {
        b = b.split('.')[0];
        // if b is negative
        if (b[0] === '-') {
          let i = b.length - 1;
          for (; b[i] === '0'; i--) b[i] = '9';
          b[i] = String(b[i] - 1);
        }
      }

      // Precision error. Result would not be correctly rounded since LN2 constant has only 1025 decimals
      const decRequired = this.#decimals + b / 3.25;
      if (decRequired > 1042) {
        err("'exp()'", "argument '" + x + "' is too big to reach the precision required");
      }

      const oldDecimals = this.#decimals;
      this.setDecimals(decRequired);

      const r = this.#arith(
        this.#naturalExponential(this.#arith(x, this.#arith(b, LN2, 'm'), 's')),
        String(BigInt(2) ** BigInt(b)),
        'm',
      );
      this.setDecimals(oldDecimals);
      return r;
    } else {
      let r = this.#arith('1', x, 'a'),
        // Take exponential
        last = 'x',
        cur = 0,
        fact = '1',
        acc = this.#arith(x, x, 'm');
      // Use Taylor/Maclaurin series
      for (let i = 2; cur < r.length; i++) {
        last = r;
        // Get new factorial
        fact = this.#arith(fact, String(i), 'm');
        r = this.#arith(r, this.#arith(acc, fact, 'd'), 'a');
        acc = this.#arith(acc, x, 'm');
        // Search for a digit that is not accurate
        for (; cur < r.length && last[cur] === r[cur]; cur++);
      }
      return r;
    }
  }

  #cosine(x) {
    let r = '1',
      last = 'x',
      cur = 0,
      fact = '1',
      acc = this.#arith(x, x, 'm'),
      sq = acc;
    // Use Taylor series
    for (let i = 2; cur <= this.#decimals; i += 2) {
      last = r;
      // Get new factorial
      fact = this.#arith(fact, this.#arith(String(i - 1), String(i), 'm'), 'm');
      r = this.#arith(r, this.#arith(acc, fact, 'd'), i % 4 ? 's' : 'a');
      acc = this.#arith(acc, sq, 'm');
      // Search for a digit that is not accurate
      for (; last[cur] === r[cur] && cur <= this.#decimals; cur++);
    }
    return r;
  }
  #sine(x) {
    let r = x,
      last = 'x',
      cur = 0,
      fact = '1',
      sq = this.#arith(x, x, 'm'),
      acc = this.#arith(sq, x, 'm');
    // Use Taylor series
    for (let i = 3; cur <= this.#decimals; i += 2) {
      last = r;
      // Get new factorial
      fact = this.#arith(fact, this.#arith(String(i - 1), String(i), 'm'), 'm');
      r = this.#arith(r, this.#arith(acc, fact, 'd'), (i - 1) % 4 ? 's' : 'a');
      acc = this.#arith(acc, sq, 'm');
      // Search for a digit that is not accurate
      for (; last[cur] === r[cur] && cur <= this.#decimals; cur++);
    }
    return r;
  }
  #inverseTangent(x) {
    /*
     * Domain: [-Infinity, Infinity]
     * Range: [-pi/2, pi/2]
     *
     * atan(x) = x - x^3/3 + x^5/5 - x^7/7 + ...
     * */

    /* Argument reduction
     * atan(x) = 2 * atan(x / (1 + sqrt(1 + x^2)))
     */
    if (cmp(x, '0.5') == 1) {
      return this.#arith(
        '2',
        this.inverseTangent(
          this.#arith(
            x,
            this.#arith(
              '1',
              this.#arith(this.#arith('1', this.#arith(x, x, 'm'), 'a'), '2', 'root'),
              'a',
            ),
            'd',
          ),
        ),
        'm',
      );
    } else {
      // Take inverseTangent
      let r = x,
        last = 'x',
        cur = 0,
        sq = this.#arith(x, x, 'm'),
        acc = x;
      // Use Taylor series
      for (let i = 3; cur <= this.#decimals; i += 2) {
        last = r;
        acc = this.#arith(acc, sq, 'm');
        r = this.#arith(r, this.#arith(acc, String(i), 'd'), (i - 1) % 4 ? 's' : 'a');
        // Search for a digit that is not accurate
        for (; last[cur] === r[cur] && cur <= this.#decimals; cur++);
      }
      return r;
    }
  }

  #factorial(x) {
    let r = '1';
    for (let i = '2'; cmp(i, x) < 1; i = String(BigInt(i) + BigInt('1'))) {
      r = String(BigInt(r) * BigInt(i));
    }
    return r;
  }
  setDecimals(n) {
    // decimals must be >= 0
    if (n < 0) err("'Base.setDecimals()'", `Invalid decimals value ${n}`);

    this.#decimals = n;
    this.#shift = '0'.repeat(this.#decimals + 1);
  }

  setAngle(value = 1) {
    value = value.toLowerCase();
    if (value !== 'degrees' && value !== 'radians') {
      err("'setAngle()'", 'Invalid unit ' + value);
    }
    this.#angles = value === 'degrees';
  }
  #someNan(...n) {
    return n.some((e) => e.isNaN());
  }
  #someInf(...n) {
    return n.some((e) => !e.isFinite());
  }
  #someZero(...n) {
    return n.some((e) => e.isZero());
  }
  #allInf(...n) {
    return n.every((e) => !e.isFinite());
  }
  #allZero(...n) {
    return n.every((e) => e.isZero());
  }
  #difSign(x, y) {
    return x.s !== y.s;
  }
  max(...arg) {
    const n = arg.map((e) => new BaseNumber(e.n || e, e.b || 10).toB());
    return n.reduce(
      (max, e) => (cmp(e.n, max.n) == 1 ? e : max),
      new BaseNumber('-Infinity'),
    );
  }

  min(...arg) {
    const n = arg.map((e) => new BaseNumber(e.n || e, e.b || 10).toB());
    return n.reduce(
      (min, e) => (cmp(e.n, min.n) == -1 ? e : min),
      new BaseNumber('Infinity'),
    );
  }

  radians(x, base = x.b || 10) {
    this.#n = new BaseNumber(x, base).toB().n;
    return new BaseNumber(this.#arith(this.#arith(this.#n, PI, 'm'), '180', 'd')).toB(
      base,
    );
  }

  // end of FUNCTIONS DECLARED AS B prototypical ----------------------------------------------------------------------------------------------------------

  /** Returns anew BaseNumberasenumber with precision digits rounded using 'exclusive' if it is true
   *
   * Number of digits after comma DO NOT depend on the 'base'
   * .g.
   *
   * Base(14.2309, 10).round(3)    =>   '14.231'
   * Base('f.a0ee', 16).round(3)   =>   'f.a0f'
   *
   */
  round(precision = 1, exclusive = false) {
    // Non-finite? or Non-float
    if (!this.isFinite() || !this.#f) return this.clone();

    const sign = this.s ? '-' : '',
      base = this.#b;

    let digits = this.#n.split('');
    sign === '-' && digits.shift();

    if (precision < 0 || isNaN(precision))
      err("'round()'", 'precision argument should be a number higher than -1');

    precision = Math.min(precision, this.#decimals);

    const dot = digits.indexOf('.'),
      nextDigit = digits[dot + 1 + precision];

    if (typeof nextDigit === 'undefined') return this.clone();

    // carry in case of overflow
    const _addOne = (p) => {
      if (digits[p] == '.') _addOne(p - 1);
      else if (getNumber(digits[p]) + 1 == base) {
        digits[p] = '0';
        p > 0 ? _addOne(p - 1) : digits.unshift('1');
      } else {
        digits[p] = numerals[getNumber(digits[p]) + 1];
      }
    };

    // Add +1 to the last precision digit if necessary
    base - !!exclusive - getNumber(nextDigit) <= (base - !!exclusive) / 2 &&
      _addOne(dot + precision);

    // Truncate non-significant digits
    digits.splice(digits.indexOf('.') + 1 + precision);
    !precision && digits.splice(-1);

    return new BaseNumber(sign + digits.join(''), this.#b, this.#decimals);
  }

  /** Returns a String representing the number with 'precision' digits after comma
   * Show extra '0' if needed
   *
   * Base(10).toFixed(3)            =>   '10.000'
   * Base('9ff'00005)'.toFixed(1)   =>   '9ff.0'
   *
   */
  toFixed(precision = this.#n.split('.').concat('')[1].length, exclusive = false) {
    // Non-finite?
    if (!this.isFinite()) return this.#n;

    precision = Math.min(precision, this.#decimals);
    if (precision < 0 || isNaN(precision))
      err("'toFixed()'", 'precision argument should be a number higher than -1');

    // Round
    const n = this.round(precision, exclusive);
    const [ints, decs] = n.n.split('.').concat('');

    // PadEnd to show extra '0'
    return ints + '.' + decs.padEnd(precision, '0');
  }

  trunc() {
    return new BaseNumber(this.#n.split('.')[0], this.#b);
  }

  toPrecision(precision = this.abs().n.length - (this.#f ? 1 : 0), exclusive = false) {
    if (!this.isFinite()) return this.#n;
    precision = Math.min(precision, this.abs().trunc().n.length + this.#decimals);
    if (precision < 1 || isNaN(precision))
      err("'toPrecision()'", 'precision argument should be a number higher than 0');

    let n = this.abs().n,
      dot = n.indexOf('.') + 1;

    dot ? dot-- : (dot = n.length);

    n = new BaseNumber('0' + '.' + n.replace('.', ''), this.#b)
      .round(precision, exclusive)
      .n.slice(2)
      .padEnd(dot > precision ? dot : precision, '0');
    n = n.substring(0, dot) + '.' + n.slice(dot);

    if (dot > precision ? dot : precision > n.length - 1)
      return new BaseNumber((this.s ? '-' : '') + n, this.#b).toExp();
    else {
      n[n.length - 1] === '.' && (n = n.substring(0, n.length - 1));
      return (this.s ? '-' : '') + n;
    }
  }

  higherThan(target, base = 10) {
    return (
      cmp(
        this.round(this.#decimals).toB().n,
        new BaseNumber(target, base).round(this.#decimals).toB().n,
      ) == 1
    );
  }

  lowerThan(target, base = 10) {
    return (
      cmp(
        this.round(this.#decimals).toB().n,
        new BaseNumber(target, base).round(this.#decimals).toB().n,
      ) == -1
    );
  }

  equalTo(target, base = 10) {
    return !cmp(
      this.round(this.#decimals).toB().n,
      new BaseNumber(target, base).round(this.#decimals).toB().n,
    );
  }

  newValue(n, base = this.#b) {
    n = new BaseNumber(n, base);
    Object.getOwnPropertyNames(n).forEach((prop) => (this[prop] = n[prop]));
    return this;
  }

  add(target, base = 10) {
    target = new BaseNumber(target, base).toB();
    // Some NaN, Infinity, Zero
    if (this.#someZero(this, target) || this.#someInf(this, target))
      return new BaseNumber(this.toB() + target, this.#b);

    return new BaseNumber(this.#arith(this.toB().n, target.n, 'a'))
      .toB(this.#b)
      .round(this.#decimals);
  }

  subtract(target, base = 10) {
    target = new BaseNumber(target, base).toB();
    // Some NaN, Infinity, Zero
    if (this.#someZero(this, target) || this.#someInf(this, target))
      return new BaseNumber(this.toB() - target, this.#b);

    return new BaseNumber(this.#arith(this.toB().n, target.n, 's'))
      .toB(this.#b)
      .round(this.#decimals);
  }

  multiply(target, base = 10) {
    target = new BaseNumber(target, base).toB();
    // Some NaN, Infinity, Zero
    if (this.#someZero(this, target) || this.#someInf(this, target))
      return new BaseNumber(this.toB() * target, this.#b);

    return new BaseNumber(this.#arith(this.toB().n, target.n, 'm'))
      .toB(this.#b)
      .round(this.#decimals);
  }

  divide(target, base = 10) {
    target = new BaseNumber(target, base).toB();
    // Some NaN, Infinity, Zero
    if (this.#someZero(this, target) || this.#someInf(this, target))
      return new BaseNumber(this.toB() / target, this.#b);

    return new BaseNumber(this.#arith(this.toB().n, target.n, 'd'))
      .toB(this.#b)
      .round(this.#decimals);
  }
  module(target, base = 10) {
    target = new BaseNumber(target, base).toB();
    // Some NaN, Infinity, Zero
    if (this.#someZero(this, target) || this.#someInf(this, target))
      return new BaseNumber(this.toB() % target, this.#b);

    return new BaseNumber(this.#arith(this.toB().n, target.n, 'mod'))
      .toB(this.#b)
      .round(this.#decimals);
  }
  pow(target, base = 10) {
    target = new BaseNumber(target, base).toB();
    // Some NaN, Infinity, Zero
    if (this.#someZero(this, target) || this.#someInf(this, target))
      return new BaseNumber(Math.pow(this.toB(), target), this.#b);

    return new BaseNumber(
      this.#arith(this.toB().n, target.n, 'pow'),
      this.#b,
      this.#decimals,
    );
  }
  root(target, base = 10) {
    target = new BaseNumber(target, base).toB();
    // Some NaN, Infinity, Zero
    if (this.#someZero(this, target) || this.#someInf(this, target))
      return new BaseNumber(Math.pow(this.toB(), 1 / target), this.#b);

    /* Determine sign (complex number) return NaN
     * Some operations that can be performed in a calculator like
     * root[3](-2) Cannot be performed since the formula would not converge for negative numbers
     */
    if (this.s) {
      return new BaseNumber('NaN', this.#b, this.#decimals);
    }

    target = target.n;
    const dot = target.indexOf('.') + 1 || target.length;
    const dec = target.length - dot;
    target = target.replace('.', '');

    // Fix precision this.#decimals
    const oldDecimals = this.#decimals;
    this.setDecimals(this.#decimals + (Math.floor((1 * this.#decimals) / 100) || 2));

    let aux = this.#arith(this.#n, target, 'root');
    aux = this.#arith(aux, '1' + '0'.repeat(dec >= 0 ? dec : 0), 'pow');
    const r = new BaseNumber(aux).toB(this.#b);
    this.setDecimals(oldDecimals);
    return r.round(this.#decimals);
  }
  sqrt() {
    return this.root(2);
  }
  cbrt() {
    return this.root(3);
  }

  fact() {
    if (!this.isFinite()) return this.clone();
    const n = this.toB().trunc().n;

    return new BaseNumber(this.factorial(n)).toB(this.#b);
  }

  toDec() {
    return this.toB();
  }
  toHex() {
    return this.toB(16);
  }
  toBin() {
    return this.toB(2);
  }
  toOct() {
    return this.toB(8);
  }

  valueOf() {
    return this.#n;
  }
  toString() {
    return this.isZero() ? this.abs().n : this.#n;
  }
  base() {
    return this.#b;
  }
  sign() {
    if (this.isZero() || this.isNaN()) return +this;
    return this.#s ? -1 : 1;
  }

  /* Returns an object with 3 keys representing the floating point representation
   * of the number in 32 or 64 bits. Sensitive to special values such as NaN or ±Infinity
   *
   * NaN         = 1 11111111 11111111111111111111111
   * Infinity    = 0 11111111 00000000000000000000000
   * -Infinity   = 1 11111111 00000000000000000000000
   *
   */
  toIEEE754(bits64 = false) {
    let bin = this.toB(2).n,
      sign = '0';
    const mantL = bits64 ? 52 : 23,
      expL = bits64 ? 11 : 8;
    // Determine sign
    bin[0] === '-' && (sign = '1') && (bin = bin.slice(1));
    // check for NaN
    if (this.isNaN()) {
      return { sign: '1', exponent: '1'.repeat(expL), mantissa: '1'.repeat(mantL) };
    }

    // check for non finite values
    if (cmp(bin, bits64 ? MAX_DOUBLE_VALUE : MAX_FLOAT_VALUE) == 1) {
      return { sign: sign, exponent: '1'.repeat(expL), mantissa: '0'.repeat(mantL) };
    }

    // Save comma index
    const dot = this.#f ? bin.indexOf('.') : bin.length;
    // Remove comma
    bin = bin.replace('.', '');
    // Exponent: how many times did comma move
    const _one = bin.indexOf('1');

    let exp = new BaseNumber(_one >= 0 ? dot - _one - 1 + (bits64 ? 1023 : 127) : 0);

    // If negative, means exp = 0
    if (exp.isNeg()) {
      exp = '0';
      bin = '0';
    } else {
      exp = exp.toB(2).n;

      // Remove left zeros
      while (bin.indexOf('1') > 0) bin = bin.slice(1);
      bin = bin.slice(1);
    }

    return {
      sign: sign,
      exponent: exp.padStart(expL, '0').slice(0, expL),
      mantissa: bin.padEnd(mantL, '0').slice(0, mantL),
    };
  }

  clone() {
    return new BaseNumber(this.#n, this.#b);
  } //new BaseNumber(P) cause overflow

  toNumber() {
    return +this.toB();
  }

  abs() {
    return new BaseNumber(this.s ? this.#n.slice(1) : this.#n, this.#b);
  }

  floor() {
    return this.s
      ? this.abs().ceil().neg()
      : new BaseNumber(this.#n.split('.')[0], this.#b);
  }

  ceil() {
    if (this.#f) {
      if (this.s) {
        return this.abs().floor().neg();
      } else {
        return new BaseNumber(
          String(
            BigInt(new BaseNumber(this.#n.split('.')[0], this.#b).toB().n) + BigInt('1'),
          ),
        ).toB(this.#b);
      }
    } else return this.clone();
  }

  clamp(min, max) {
    return this.min(this.max(this, min), max);
  }

  neg() {
    return new BaseNumber(this.s ? this.#n.slice(1) : '-' + this.#n, this.#b);
  }

  isNeg() {
    return this.s;
  }
  isPos() {
    return !this.s;
  }
  isInt() {
    return !this.#f;
  }
  isFloat() {
    return this.#f;
  }
  isBase(base = 10) {
    return this.#b === parseInt(base);
  }
  isZero() {
    return this.abs().n === '0';
  }
  isFinite() {
    return this.abs().n !== 'Infinity' && !this.isNaN();
  }
  isNaN() {
    return this.#n === 'NaN';
  }
  toExp(digits = -1) {
    return this.toExponential(digits);
  }
  toExponential(digits = -1) {
    if (!this.isFinite()) return this.#n;
    let n = this.abs().n;
    let exp = (this.#f ? n.indexOf('.') : n.length) - 1;
    n = n.replace('.', '');

    while (n[0] === '0' && n.length !== 1) (n = n.slice(1)) && exp--;

    // by default show all digits
    digits = Math.floor(digits);
    if (digits < 0) digits = n.length;
    else digits++;
    // fill extra zeros if needed
    n = n.padEnd(digits, '0');

    return (
      (this.s ? '-' : '') +
      n[0] +
      '.' +
      n.slice(1, digits) +
      ' e' +
      (exp > -1 ? '+' + exp : exp)
    );
  }

  /*  ln(-n)        = NaN
   *  ln(0)         = -Infinity
   *  ln(-0)        = -Infinity
   *  ln(1)         = 0
   *  ln(Infinity)  = Infinity
   *  ln(-Infinity) = NaN
   *  ln(NaN)       = NaN
   */
  ln() {
    // If number is zero return -Infinity
    if (this.isZero()) return new BaseNumber('-Infinity', this.#b);
    // If negative return NaN
    if (this.s) return new BaseNumber('NaN', this.#b);
    // If +Infinity
    if (!this.isFinite()) return this.clone();
    // Fix precision decimals
    const oldDecimals = this.#decimals;
    this.setDecimals(this.#decimals + (Math.floor(this.#decimals / 100) || 2));

    const r = new BaseNumber(this.#naturalLogarithm(this.toB().n)).toB(this.#b);

    this.setDecimals(oldDecimals);
    return r.round(this.#decimals);
  }

  log(y = 10) {
    // If number is negative or zero return NaN
    if (cmp(this.#n, '0') != 1) return new BaseNumber('NaN', this.#b);
    // If +Infinity
    if (!this.isFinite()) return this.clone();
    // Fix precision this.#decimals
    const oldDecimals = this.#decimals;
    this.setDecimals(this.#decimals + (Math.floor(this.#decimals / 100) || 2));

    const r = new BaseNumber(
      this.#arith(
        this.#naturalLogarithm(this.toB().n),
        this.#naturalLogarithm(String(y)),
        'd',
      ),
    ).toB(this.#b);

    this.setDecimals(oldDecimals);
    return r.round(this.#decimals);
  }

  exp() {
    // If Infinity/NaN return the object cloned
    if (!this.isFinite()) return this.clone();
    // Fix precision decimals
    const oldDecimals = this.#decimals;
    this.setDecimals(this.#decimals + (Math.floor(this.#decimals / 100) || 2));
    const r = new BaseNumber(this.#naturalExponential(this.toB().n)).toB(this.#b);

    this.setDecimals(oldDecimals);
    return r.round(this.#decimals);
  }

  cos() {
    return this.cosine();
  }
  /* cos(0)         = 1
   * cos(-0)        = 1
   * cos(Infinity)  = NaN
   * cos(-Infinity) = NaN
   * cos(NaN)       = NaN
   */
  cosine() {
    if (this.isZero()) return new BaseNumber('1', this.#b);
    if (!this.isFinite()) return new BaseNumber('NaN', this.#b);

    // Fix precision decimals
    const oldDecimals = this.#decimals,
      x = this.toB().n;
    this.setDecimals(this.#decimals + (Math.floor(this.#decimals / 100) || 2));

    // If angle is in degrees transform to radians
    const r = new BaseNumber(
      this.cosine(
        this.#angles
          ? this.#arith(this.#arith(this.#arith(x, PI, 'm'), '180', 'd'), '180', 'mod')
          : this.#arith(x, PI, 'mod'),
      ),
    ).toB(this.#b);

    this.setDecimals(oldDecimals);
    return r.round(this.#decimals);
  }

  /* sin(0)         = 0
   * sin(-0)        = -0
   * sin(Infinity)  = NaN
   * sin(-Infinity) = NaN
   * sin(NaN)       = NaN
   */
  sin() {
    return this.sine();
  }
  sine() {
    if (this.isZero()) return this.clone();
    if (!this.isFinite()) return new BaseNumber('NaN', this.#b);

    // Fix precision decimals
    const oldDecimals = this.#decimals,
      x = this.toB().n;
    this.setDecimals(this.#decimals + (Math.floor(this.#decimals / 100) || 2));

    // If angle is in degrees transform to radians
    const r = new BaseNumber(
      this.sine(
        this.#angles
          ? this.#arith(this.#arith(this.#arith(x, PI, 'm'), '180', 'd'), '180', 'mod')
          : this.#arith(x, PI, 'mod'),
      ),
    ).toB(this.#b);

    this.setDecimals(oldDecimals);
    return r.round(this.#decimals);
  }

  /* tan(0)         = 0
   * tan(-0)        = -0
   * tan(Infinity)  = NaN
   * tan(-Infinity) = NaN
   * tan(NaN)       = NaN
   * tan(90)        = Infinity
   * tan(-90)       = -Infinity
   */
  tan() {
    return this.tangent();
  }
  tangent() {
    if (this.isZero()) return this.clone();
    if (!this.isFinite()) return new BaseNumber('NaN', this.#b);

    // Fix precision decimals
    const oldDecimals = this.#decimals,
      x = this.toB().n;
    this.setDecimals(this.#decimals + (Math.floor(this.#decimals / 100) || 2));

    // If angle is in degrees transform to radians
    const radians = this.#angles
      ? this.#arith(this.#arith(this.#arith(x, PI, 'm'), '180', 'd'), '180', 'mod')
      : this.#arith(x, PI, 'mod');

    let _cos = this.cosine(radians);
    // Check if cosine is '0'
    let [i, d] = _cos.split('.').concat('x');
    d = d.slice(0, oldDecimals);
    // Trail zeros at right
    while (d[d.length - 1] === '0') d = d.slice(0, -1);
    // If Infinity
    if (!d) {
      if (i === '0') return new BaseNumber('Infinity', this.#b);
      else if (i === '-0') return new BaseNumber('-Infinity', this.#b);
    }

    const r = new BaseNumber(this.#arith(this.sine(radians), _cos, 'd')).toB(this.#b);

    this.setDecimals(oldDecimals);
    return r.round(this.#decimals);
  }

  /* acos(0)       = pi/2
   * acos(-0)      = pi/2
   * acos(1)       = 0
   * acos(-1)      = pi
   * acos(1/2)     = pi/3
   * acos(-1/2)    = 2*pi/3
   * acos(|x| > 1) = NaN
   * acos(NaN)     = NaN
   *
   * acos(x) = pi/2 - asin(x)
   */
  acos() {
    return this.inverseCosine();
  }
  inverseCosine() {
    if (this.isNaN() || this.isZero()) return this.clone();
    if (cmp(this.abs().n, '1') == 1) return new BaseNumber('NaN', this.#b);

    // Fix precision decimals
    const oldDecimals = this.#decimals,
      x = this.toB().n;
    this.setDecimals(this.#decimals + (Math.floor((3 * this.#decimals) / 100) || 2));

    // Get angle in radians
    const radians = this.#arith(
      this.#arith('0.5', PI, 'm'),
      this.#arith(
        '2',
        this.inverseTangent(
          this.#arith(
            x,
            this.#arith(
              '1',
              this.#arith(this.#arith('1', this.#arith(x, x, 'm'), 's'), '2', 'root'),
              'a',
            ),
            'd',
          ),
        ),
        'm',
      ),
      's',
    );

    // Transform angle to degrees if necessary
    const r = new BaseNumber(
      this.#angles ? this.#arith(this.#arith('180', radians, 'm'), PI, 'd') : radians,
    ).toB(this.#b);

    this.setDecimals(oldDecimals);
    return r.round(this.#decimals);
  }

  /* asin(0)       = 0
   * asin(-0)      = -0
   * asin(1/2)     = pi/6
   * asin(-1/2)    = -pi/6
   * asin(1)       = pi/2
   * asin(-1)      = -pi/2
   * asin(|x| > 1) = NaN
   * asin(NaN)     = NaN
   *
   * asin(x) = 2*atan(x/(1 + sqrt(1 - x^2)))
   */
  asin() {
    return this.inverseSine();
  }
  inverseSine() {
    if (this.isNaN() || this.isZero()) return this.clone();
    if (cmp(this.abs().n, '1') == 1) return new BaseNumber('NaN', this.#b);

    // Fix precision decimals
    const oldDecimals = this.#decimals,
      x = this.toB().n;
    this.setDecimals(this.#decimals + (Math.floor((3 * this.#decimals) / 100) || 3));

    // Get angle in radians
    const radians = this.#arith(
      '2',
      this.inverseTangent(
        this.#arith(
          x,
          this.#arith(
            '1',
            this.#arith(this.#arith('1', this.#arith(x, x, 'm'), 's'), '2', 'root'),
            'a',
          ),
          'd',
        ),
      ),
      'm',
    );

    // Transform angle to degrees if necessary
    const r = new BaseNumber(
      this.#angles ? this.#arith(this.#arith('180', radians, 'm'), PI, 'd') : radians,
    ).toB(this.#b);

    this.setDecimals(oldDecimals);
    return r.round(this.#decimals);
  }

  // STTTTTTTTTTTTTTTTTTTTTTTTTTTTTOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPPP
  /* atan(0)         = 0
   * atan(-0)        = -0
   * atan(1)         = pi/4
   * atan(-1)        = -pi/4
   * atan(Infinity)  = pi/2
   * atan(-Infinity) = -pi/2
   * atan(NaN)       = NaN
   */
  atan() {
    return this.inverseTangent();
  }
  inverseTangent() {
    if (this.isNaN() || this.isZero()) return this.clone();
    if (!this.isFinite()) return new BaseNumber(this.#arith(PI, '2', 'd'), this.#b);

    // Fix precision this.#decimals
    const oldDecimals = this.#decimals,
      x = this.toB().n;

    this.setDecimals(this.#decimals + (Math.floor((3 * this.#decimals) / 100) || 3));

    // Get angle in radians
    const radians = this.inverseTangent(x);

    // Transform angle to degrees if necessary
    const r = new BaseNumber(
      this.#angles ? this.#arith(this.#arith('180', radians, 'm'), PI, 'd') : radians,
    ).toB(this.#b);

    this.setDecimals(oldDecimals);
    return r.round(this.#decimals);
  }

  toB(base = 10) {
    return this.toBase(base);
  }
  toBase(base = 10) {
    base = parseInt(base);
    if (this.#b === base || !this.isFinite()) return this.clone();
    const n = this.abs().n;
    _catch('0', base, "'toBase()'");

    // Fix precision this.#decimals
    const oldDecimals = this.#decimals;
    this.setDecimals(this.#decimals + (Math.floor(this.#decimals / 100) || 2));

    let acc = '1',
      int =
        this.#b === 10
          ? n.split('.')[0]
          : n
              .split('.')[0]
              .split('')
              .reduceRight((a, d) => {
                const r = String(BigInt(a) + BigInt(getNumber(d)) * BigInt(acc));
                acc = String(BigInt(acc) * BigInt(this.#b));
                return r;
              }, 0),
      parsedInt = [];

    // While int >= base
    while (cmp(int, String(base)) > -1) {
      const toPush = String(BigInt(int) % BigInt(base));
      parsedInt.unshift(numerals[toPush]);
      int = String(BigInt(int) / BigInt(base));
    }
    parsedInt.unshift(numerals[int]);

    acc = String(this.#b);
    let dec = this.#f
      ? this.#b === 10
        ? '0.' + n.split('.')[1]
        : n
            .split('.')[1]
            .split('')
            .reduce((a, d) => {
              const r = this.#arith(a, this.#arith(String(getNumber(d)), acc, 'd'), 'a');
              acc = String(BigInt(acc) * BigInt(this.#b));
              return r;
            }, '0')
      : '0';
    let timeout = this.#decimals + 1,
      parsedDec = '',
      toPush = '1',
      dot = dec.split('.').concat('')[1].length;
    dec = dec.replace('.', '');
    while (timeout && toPush && dec !== '0') {
      dec = String(BigInt(dec.slice(-dot)) * BigInt(base));
      toPush = dec.slice(0, -dot) || '0'; // "0" cause toPush must be a truely value
      parsedDec += numerals[toPush];
      timeout--;
    }
    return new BaseNumber(
      (this.s ? '-' : '') + parsedInt.join('') + '.' + parsedDec,
      base,
      this.#decimals,
    );
  }
  [Symbol.for('nodejs.util.inspect.custom')]() {
    return this.toString();
  }
  get [Symbol.toStringTag]() {
    return 'BaseNumber';
  }
}

function getNumber(e) {
  return numerals.indexOf(e);
}

/** @functionNormalize string
 *
 * Replace commas by dot and remove spaces
 * Simplify signs +/- at the beginning
 *
 * @param (string)
 * @return string normalized
 */
function Normalize(s) {
  s = String(s).split(',').join('.').split('_').join('').split(' ').join('');
  s !== 'NaN' && s.indexOf('Infinity') === -1 && (s = s.toLowerCase());
  s.indexOf('.') + 1 == s.length && (s = s.substring(0, s.length - 1));
  while (s[0] === '+' || s[0] === '-') {
    while (s[0] === '+') s = s.slice(1);
    if (s[0] === '-') {
      if (s[1] === '-') s = s.slice(2);
      else if (s[1] === '+') s = '-' + s.slice(2);
      else break;
    }
  }
  return s;
}
function err(func, msg) {
  throw new Error('BaseDecimal Erro! in ' + func + ': ' + msg);
}
/** @functionFix notation
 *
 * If user writes a number with scientific notation the function
 * simplify it to a string with 0 at the left or right
 *
 * Remove trailing zeros and fix dot notation
 *
 * @param (string)
 * @return string without notation
 */
function fixNott(n) {
  const sign = n[0] === '-';
  sign && (n = n.slice(1));
  let exp;
  if ((n.indexOf('e+') >= 0 && (exp = 'e+')) || (n.indexOf('e-') >= 0 && (exp = 'e-'))) {
    n = n.split(exp);
    n[0] = n[0].split('');
    let dot = n[0].indexOf('.');
    dot >= 0 ? n[0].splice(dot, 1) : (dot = n[0].length);
    const moveDot = parseInt(n[1], 10);

    if (isNaN(moveDot)) err("'constructor'", 'invalid exponent');

    try {
      // Move the comma/dot
      if (exp === 'e+') {
        if (moveDot + dot < n[0].length) n[0].splice(moveDot + dot, 0, '.');
        else if (moveDot + dot > n[0].length)
          n[0].push('0'.repeat(moveDot + dot - n[0].length));
      } else {
        if (dot - moveDot > 0) n[0].splice(dot - moveDot, 0, '.');
        else if (dot - moveDot <= 0)
          n[0].unshift('0.' + '0'.repeat(Math.abs(dot - moveDot)));
      }
      n = n[0].join('');
    } catch (e) {
      err("'constructor'", 'max number length exceded');
    }
  }
  n = n.split('');
  // remove extra '0' at right
  while (n[0] === '0' && n[1] !== '.') n.shift();
  // add '0' at left if has a dot at index 0
  n[0] === '.' && n.unshift('0');
  // remove extra '0' at right if is float, or extra '.'
  while (n.indexOf('.') >= 0 && (n[n.length - 1] === '0' || n[n.length - 1] === '.'))
    n.pop();
  return (sign ? '-' : '') + (n.length ? n.join('') : '0');
}
function _catch(n, base, func) {
  try {
    parseInt(base);
    if (Number.isNaN(base) || base < 2 || base > 36) throw new Error();
  } catch (e) {
    err(`_catch first parseInt(base)@ ${func}`, `For n=${n} invalid base=${base}.`);
  }
  try {
    parseFloat(n);
  } catch (e) {
    if (typeof n === 'string') {
      let digits = n
        .replace('NaN', '0')
        .replace('Infinity', '0')
        .replace('e+', '')
        .replace('e-', '')
        .split('');
      if (
        n === '' ||
        digits.reduce((a, d) => (d == '.' ? ++a : a), 0) > 1 ||
        digits.reduce((a, d) => (d == '-' ? ++a : a), 0) > 1 ||
        digits.some(
          (d) =>
            Number.isNaN(d) &&
            (getNumber(d) < 10 || getNumber(d) > 36) &&
            d != '.' &&
            d != '-',
        )
      )
        err(func, 'invalid number ' + n);
      if (digits.some((d) => getNumber(d) >= base))
        err(func, "number '" + n + "' doesn't match base " + base);
    } else {
      err(
        `_catch 2nd parseFloat(n) if not string @ ${func}`,
        `For invalid n=${n} with base=${base}.`,
      );
    }
  }
}
/** @functionLexicographic comparition
 *
 * When working with strings cannot compare like numbers, so need to
 * make them the same length and add '0' on the left and on the right to
 * compare each character
 *
 * It catches special cases such as NaN and ±Infinity
 *
 * @param (string, string)
 * @return 1, -1 or 0 depends on the comparition result
 */
function cmp(s1, s2) {
  // Fix sign
  let sign1 = '',
    sign2 = '';
  s1[0] === '-' && (sign1 = '0') && (s1 = s1.slice(1));
  s2[0] === '-' && (sign2 = '0') && (s2 = s2.slice(1));

  // check special values
  if (s1.indexOf('Infinity') + 1) {
    if (s1 === s2) return sign1 < sign2;
    return sign1 ? -1 : 1;
  }
  if (s2.indexOf('Infinity') + 1) {
    if (s1 === s2) return sign1 > sign2;
    return sign2 ? 1 : -1;
  }

  //  add point in case there isn't
  s1 += s1.indexOf('.') < 0 ? '.0' : '';
  s2 += s2.indexOf('.') < 0 ? '.0' : '';

  // Add '0' on the left
  s1 = s1.split('.')[0].padStart(s2.indexOf('.'), '0') + '.' + s1.split('.')[1];
  s2 = s2.split('.')[0].padStart(s1.indexOf('.'), '0') + '.' + s2.split('.')[1];
  // Add '0' on the right
  s1 = sign1 + s1.padEnd(s2.length, '0');
  s2 = sign2 + s2.padEnd(s1.length, '0');

  return s1 > s2 || -(s1 < s2);
}

/** In order to move the comma we need to remove the sign to avoid errors like '0000-0.56' */
function arith_fixDot(s, cond) {
  const sign = s[0] === '-' ? '-' : '';
  sign && (s = s.slice(1));
  s = s.padStart(cond, '0');
  return sign + s.slice(0, s.length - cond) + '.' + s.slice(s.length - cond);
}

function _pow(a, b, decimals, instance) {
  // To avoid undefined due to trailing right zeros
  if (a === '0') return a;
  // If b is integer make normal power
  if (b.indexOf('.') == -1 && a.indexOf('.') == -1) {
    // Remove "0" at right to short time execution
    let tZeros = 0;
    for (; a[a.length - 1] === '0'; tZeros++) a = a.slice(0, -1);
    return String(BigInt(a) ** BigInt(b)) + (tZeros ? '0'.repeat(tZeros).repeat(b) : '');
    //If 'a' or 'b' are floats do exp(b * ln(a))
  } else {
    const oldDecimals = decimals,
      extraDec = Math.floor(b * 1.3 * Math.log10(a));
    if (decimals + extraDec > 1042) {
      err("'pow()'", 'arguments are too big to reach the precision required');
    }
    instance.setDecimals(decimals + extraDec);
    const r = instance.___natExp(instance.___arith(b, instance.___natLog(a), 'm'));
    instance.setDecimals(oldDecimals);
    return r;
  }
}

export default BaseNumber;
