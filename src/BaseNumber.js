...
(function(globalScope) {
    ...
    B.LN10 = '2.';
    // The natural logarithm of 2 (1025 digits).
    B.LN2 = '0.';
    B.PI = '3.';
    B.e = '2.';
    ...
    const _catch = (n, base, func) => {
        ...
        if (digits.some(d => B.getNumber(d) >= base)) err(func, "number doesn´t match base");
    ...
    function cmp(s1, s2) {
        ...
        let sign1 = "",
            sign2 = "";
        s1[0] === "-" && (sign1 = "-") && (s1 = s1.slice(1));
        s2[0] === "-" && (sign2 = "-") && (s2 = s2.slice(1));
        s1 += s1.indexOf(".") < 0 ? ".0" : "";
        s2 += s2.indexOf(".") < 0 ? ".0" : "";
    ...
    const arith = (b1, b2, op) => {
        ...
        if (op === "pow") return fixNott(_intPow(b1, b2));
        if (op === "root") return fixNott(_intPow(b1, arith("1", b2, "d")));
        ...
        function _intPow(a, b) {
            // To avoid stackoverflow
            if (b === "0" || a === "1") { return "1"; }
            // If b is integer make normal power
            if (b.indexOf(".") == -1) {
                // If a and is integer remove "0" at right to short time execution
                let tZeros = 0;
                if (a.indexOf(".") == -1)
                    for (; a[a.length - 1] === "0"; tZeros++) a = a.slice(0, -1);

                const shiftAdec = (a.split(".").concat("")[1].length) * b || a.length;
                a = a.replace(".", "");

                const numerator = fixDot(String(BigInt(a) ** BigInt(b)), shiftAdec);

                return numerator + (tZeros ? "0".repeat(tZeros).repeat(b) : "");
    ...
    function naturalLogarithm(x) {
        ...
        // If number <= 0.9
        if (cmp(x, "0.9") != 1) {
            // Trail zeros at left to convert any decimal into an integer
        ...
        if (cmp(a, "1.1") > -1) {
            ...
            y < 1 && (y = 1);
            const d = String(2 ** y);
            r = arith(naturalLogarithm(arith(a, d, "d")), arith(String(y), B.LN2, "m"), "a");
        ...
        return arith(r, arith(String(b), B.LN10, "m"), "a");
    }
    ...
    B.radians = (x, base = 10) => {
        ...
        return new B(arith(arith(n, B.PI, "m"), "180", "d")).toB(base);
    }
    ...
    function B(n, base = 10) {
        ...
        P.round = function(precision, exclusive = false) {
            const addOverflow = p => {
        ...
        P.higherThan = function(target, base = 10) {
            return cmp(P.toB().n, new B(target, base).toB().n) == 1;
        }
        ...
        P.equalTo = function(target, base = 10) {
            return !cmp(P.toB().n, new B(target, base).toB().n);
        }
        ...
        P.root = function(target, base = 10) {
             ...
             * root[3](-2) Cannor be performed since the formula would not converge for negative numbers
             */
            if (P.s) {
        ...
        P.clone = function() { return new B(P.n, P.b); }
        ...
        P.toExp = function() {
            ...
            return (P.s ? "-" : "") + fixNott(n[0] + "." + n.slice(1)) + " e" + (exp > -1 ? "+" + exp : exp);
        }
        ...
        P.cosine = P.cos = function() {
            ...
            // If angle is in grades transform to radians
            const r = new B(cosine(angles ? arith(arith(arith(P.toB().n, B.PI, "m"), "180", "d"), "180", "mod") : arith(P.toB().n, B.PI, "mod"))).toB(P.b);
        ...
        P.sine = P.sin = function() {
            ...
            // If angle is in grades transform to radians
            const r = new B(sine(angles ? arith(arith(arith(P.toB().n, B.PI, "m"), "180", "d"), "180", "mod") : arith(P.toB().n, B.PI, "mod"))).toB(P.b);
        ...
        P.tangent = P.tan = function() {
            ...
            if (P.isZero()) return P.clone();
            if (!P.isFinite() || P.abs().n === "90") return new B("NaN", P.b);
            ...
            // If angle is in grades transform to radians
            const radians = angles ? arith(arith(arith(P.toB().n, B.PI, "m"), "180", "d"), "180", "mod") : arith(P.toB().n, B.PI, "mod");

            const r = new B(arith(sine(radians), cosine(radians), "d")).toB(P.b);
